...
module.exports = async (client) => {
 ...
 if (interaction.commandName === 'ticket-ekle') {
    if(interaction.member.roles.cache.has(db.get(interaction.guild.id + "+rol"))) {
      ...
      if (!user && !role) {
          ...
          if (user) {
            try {
              interaction.channel.permissionOverwrites.create(user, { ViewChannel: true });
            } catch (err) {
          ...
          if (role) {
            try {
              interaction.channel.permissionOverwrites.create(role, { ViewChannel: true });
            } catch (err) {
                const embed = new EmbedBuilder()
                .setTitle("Hata İle Karşılaşıldı! Hata Tarihi:" +moment().format('DD MMMM YYYY HH:mm'))
                .setDescription("````" + err + "```")
          ...
          interaction.reply({ content: "İlgili Rol ve/veya Üye Eklendi!", ephemeral: true });
      }
    } else {
      interaction.reply({content: "Bu Komutu Kullanabilmek İçin Yetkin Yok!", ephemeral: true})
    }
    console.log(chalk.greenBright(`[BİLGİ] Ticket Ekle Sistemi Başarıyla Yüklendi!`)); 
             }
